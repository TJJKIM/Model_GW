...
subroutine HFChemPgivenN(SCFloop,SpinSplit,Dimen,t1,U1,ChemP,tq,U,Vq,iWnlist,K1list,K2list,K3list,Taulist,R1list,R2list,R3list,iWnGridNum,K1GridNum,K2GridNum,K3GridNum,TauGridNum,R1GridNum,R2GridNum,R3GridNum,Noccup,ChemOccup,MatDim,delta,beta,Ntot,ChemTH,NonIntGreenf_k_iWn,Hartree_k_iWn,Fock_k_iWn,dummy_k_iWn)
Implicit none
!argument
integer, intent(in) :: SCFloop,Dimen,K1GridNum,K2GridNum,K3GridNum,iWnGridNum,TauGridNum,R1GridNum,R2GridNum,R3GridNum, MatDim
real(8) ,intent(in) ::  delta, SpinSplit(MatDim/2),beta,Noccup(MatDim),Ntot,ChemTH,t1(MatDim,MatDim),U1(MatDim,MatDim),U(MatDim,MatDim)
...
complex(8), intent(in) :: iWnlist(iWnGridNum),tq(K1GridNum,K2GridNum,K3GridNum,MatDim,MatDim),Vq(K1GridNum,K2GridNum,K3GridNum,MatDim,MatDim),NonIntGreenf_k_iWn(K1GridNum,K2GridNum,K3GridNum,iWnGridNum,MatDim,MatDim),  Hartree_k_iWn(K1GridNum,K2GridNum,K3GridNum,iWnGridNum,MatDim,MatDim), Fock_k_iWn(K1GridNum,K2GridNum,K3GridNum,iWnGridNum,MatDim,MatDim)
...
ChemP=-2*Dimen*abs(maxval(t1))-2*Dimen*abs(maxval(U))-2*Dimen*abs(maxval(U1))

call  NewG_HF(SCFloop,SpinSplit,Dimen,ChemP,K1GridNum,K2GridNum,K3GridNum,iWnGridNum,TauGridNum,R1GridNum,R2GridNum,R3GridNum,MatDim,beta,K1list,K2list,K3list,iWnlist,Taulist,R1list,R2list,R3list,NonIntGreenf_k_iWn,Hartree_k_iWn,Fock_k_iWn,dummy_k_iWn)
call G_k_iWn_Occup2(Dimen,K1list,K2list,K3list,iWnlist,K1GridNum,K2GridNum,K3GridNum,iWnGridNum,tq,U,Vq,Noccup,ChemOccup,MatDim,delta,ChemP,beta,dummy_k_iWn)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine NewG_HF(SCFloop,SpinSplit,Dimen,ChemP,K1GridNum,K2GridNum,K3GridNum,iWnGridNum,TauGridNum,R1GridNum,R2GridNum,R3GridNum,MatDim,beta,K1list,K2list,K3list,iWnlist,Taulist,R1list,R2list,R3list,NonIntGreenf_k_iWn,Hartree_k_iWn,Fock_k_iWn,dummy_k_iWn)

Implicit none

! argument
integer, intent(in) :: SCFloop,Dimen,K1GridNum,K2GridNum,K3GridNum,iWnGridNum,TauGridNum,R1GridNum,R2GridNum,R3GridNum,MatDim
real(8), intent(in) :: SpinSplit(MatDim/2),K1list(K1GridNum,3),K2list(K2GridNum,3),K3list(K3GridNum,3), Taulist(TauGridNum), R1list(R1GridNum,3),R2list(R2GridNum,3),R3list(R3GridNum,3),beta,ChemP
complex(8), intent(in) :: iWnlist(iWnGridNum), NonIntGreenf_k_iWn(K1GridNum,K2GridNum,K3GridNum,iWnGridNum,MatDim,MatDim),  Hartree_k_iWn(K1GridNum,K2GridNum,K3GridNum,iWnGridNum,MatDim,MatDim), Fock_k_iWn(K1GridNum,K2GridNum,K3GridNum,iWnGridNum,MatDim,MatDim)
complex(8), intent(inout) :: dummy_k_iWn(K1GridNum,K2GridNum,K3GridNum,iWnGridNum,MatDim,MatDim)

! local variable
integer i,j,kx,ky,kz,l,E,n,o
complex(8) Iden(MatDim,MatDim),SpinSplitMat(MatDim,MatDim),dummyH(MatDim,MatDim),dummyG(MatDim,MatDim), dummy(MatDim,MatDim),dummy2(MatDim,MatDim),dummy3(MatDim,MatDim)

SpinSplitMat=(0.0d0,0.0d0)
do o=1,(MatDim/2)
     SpinSplitMat(o,o)=SpinSplit(o)/2.0d0
     SpinSplitMat((MatDim/2)+o,(MatDim/2)+o)=-SpinSplit(o)/2.0d0
enddo
          ...
          do ky =1,(K2GridNum)
               do kz =1,(K3GridNum)
                    do n=1,iWnGridNum
                         ...
                         if (SCFloop==1) then
                              dummy2(:,:)=dummy(:,:)-Hartree_k_iWn(kx,ky,kz,n,:,:)-Fock_k_iWn(kx,ky,kz,n,:,:)+ChemP*Iden+SpinSplitMat
                              !print *, dummy2
                         else
                              dummy2(:,:)=dummy(:,:)-Hartree_k_iWn(kx,ky,kz,n,:,:)-Fock_k_iWn(kx,ky,kz,n,:,:)+ChemP*Iden
                         endif
     ...
     do kx =1,(K1GridNum)
          do ky =1,(K2GridNum)
               do n=1,iWnGridNum
                    ...
                    if (SCFloop==1) then
                         dummy2(:,:)=dummy(:,:)-Hartree_k_iWn(kx,ky,1,n,:,:)-Fock_k_iWn(kx,ky,1,n,:,:)+ChemP*Iden+SpinSplitMat
                         !print *, dummy2
                    else
                         dummy2(:,:)=dummy(:,:)-Hartree_k_iWn(kx,ky,1,n,:,:)-Fock_k_iWn(kx,ky,1,n,:,:)+ChemP*Iden
                    endif
...
if (Dimen==1) then
     do kx =1,(K1GridNum)
          do n=1,iWnGridNum
               ...
               if (SCFloop==1) then
                    dummy2(:,:)=dummy(:,:)-Hartree_k_iWn(kx,1,1,n,:,:)-Fock_k_iWn(kx,1,1,n,:,:)+ChemP*Iden+SpinSplitMat
                    !print *, dummy2
                    !stop
               else
                    dummy2(:,:)=dummy(:,:)-Hartree_k_iWn(kx,1,1,n,:,:)-Fock_k_iWn(kx,1,1,n,:,:)+ChemP*Iden
               endif
